/**
 * js/particles.js - Universe Engine
 */
    ...
    try {
        ...
        class Star {
            ...
            update(timeScale) {
                ...
                this.x += this.driftX * timeScale; this.y += this.driftY * timeScale;
        ...
        class ShootingStar {
            ...
            spawn() {
                this.active = true;
                this.x = Math.random() * w; this.y = Math.random() * (h * 0.3);
        ...
        // Local Observer protects CPU when scrolling to Fireworks
        if ('IntersectionObserver' in window) {
            const obs = new IntersectionObserver((entries) => {
                isVisible = !entries[0].isIntersecting; // Pause universe if fireworks scene is showing
            }, { threshold: 0.1 });
            const s9 = document.getElementById('scene-9');
            if (s9) obs.observe(s9);
        }

        window.renderUniverse = (timeScale) => {
            if (!isVisible) return;
            ctx.fillStyle = '#030508'; 
            ctx.fillRect(0, 0, w, h);
            for (let i = 0; i < MAX_STARS; i++) {
                stars[i].update(timeScale); stars[i].draw(ctx);
            }
            if (!shootingStar.active && Math.random() < (0.001 * timeScale)) shootingStar.spawn();
            shootingStar.update(timeScale); shootingStar.draw(ctx);
        };
    } catch(e) {}
});
